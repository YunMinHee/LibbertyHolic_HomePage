...
<body>
 ...
 -->
		...
		<!-- 상단 작은 메뉴 -->
		<div id="upper_menu">
			<div class="upper_menu_item login">
				<a href="login.jsp">로그인</a>
			</div>
			<div class="upper_menu_item join">
				<a href="join.jsp">회원가입</a>
			</div>
			<div class="upper_menu_item my_space">
				<a href="#">내 공간</a>
			</div>
		</div>
		<!-- 로그인 후 -->
		...
		<!-- 게시판 -->
		<div id="wrttable" class="container">
			<div class="row">
				<%
				if (userID != null && userID.equals(bbs.getUserID())) {
				%>
				<a href="update.jsp?bbsID=<%=bbsID%>"
					class="btn btn-primary left-rewrite">수정</a> <a
					href="deleteAction.jsp?bbsID=<%=bbsID%>"
					class="btn btn-primary left-delete">삭제</a>
				<%
				}
				%>
		...
		+ bbs.getBbsDate().substring(14, 16) + "분 "%></td>
						...
						<tr>
							<td>내용</td>
							<td colspan="2"><div
									style="min-height: 100px; text-align: left;"><%=bbs.getBbsContent().replace(" ", "$bnsp;").replace("<", "&lt;").replace(">", "&gt;").replace("/n", "<br />")%></div></td>
						</tr>
					</tbody>
				</table>
				<a href="bbsTmpt.jsp" class="btn btn-primary detail_menu">목록</a>
			</div>
		</div>
	</div>
</body>
